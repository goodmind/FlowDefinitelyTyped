declare module "fluxible" {
  import type { Dispatcher, DispatcherInterface, StoreClass } from "dispatchr";

  declare export interface FluxibleConfiguration {
    /**
   ...
   * resources.
   */
  declare export class Fluxible {
    /**
 * @param options
 * @example var app = new Fluxible({
component: require('./components/App.jsx')
});
 */
    constructor(options?: FluxibleConfiguration): this;
 ...
 * @param Method called after context is created to allow
dynamically plugging the context
 * @param Method called to serialize the plugin settings to be persisted
to the client
 * @param Method called to rehydrate the plugin settings from the server
 */
 ...
 * @param Dehydrated app plugin state
 * @param Dehydrated context state. See FluxibleContext's
 ...
 */
   ...
   */
  declare export class FluxibleContext {
    /**
     ...
     */
    getStore<T: BaseStore>(store: {
      new(dispatcher: DispatcherInterface): T
    }): T;
  }
  declare export class ActionContext {
    /**
  ...
  declare export class ComponentContext {
    /**
     * Proxy function to execute action
     * @param action function that will be executed
     * @param payload
  ...
  declare export class StoreContext {}
}
