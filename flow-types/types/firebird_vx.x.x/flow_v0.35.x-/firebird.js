 ...
 * @desc You may set callback for event with
...
/**
 * @summary Synchronously commits current transaction.
 * @desc Notes:
There is only one transaction associated with connection.
Transacation is automatically started before any query if connection does not have active transaction (check
 * @see inTransaction property).
You also should note that DDL statements (altering database structure) are commited automatically.
To run quieries in context of other transaction use
 ...
 * @desc The default transaction should be not in started state before call to this method.
Read notes in
 * @see commitSync() .
 */
startSync(): void;
...
/**
 * @desc Here is Firebird to Node data type accordance:
...
| BLOB      | 	FBblob |
 */
declare type DataType = Date | string | number | FBBlob;
 ...
 * @see DataType
 */
declare class FBResult  {

/**
 * @summary Synchronously fetches result rows.
 * @desc If you pass "all" as rowCount - it will fetch all result rows.
...
fetchSync(
rowCount: number | "all",
asObject: boolean): DataType[][] | Array<{
...
fetchSync<T: {}>(rowCount: number | "all", asObject: true): T[];
...
rowCount: number | "all",
asObject: boolean,
rowCallback: (row: DataType[] | {
[column: string]: DataType
}) => void,
eofCallback: (err: Error | null, eof: boolean) => void): void;
fetch(
rowCount: number | "all",
asObject: false,
 ...
 * @desc To get instance of this object call
 * @see startNewTransactionSync or
 * @see startNewTransaction methods of
 ...
 * @desc The transaction should be not in started state before call to this method.
Read notes in
 * @see commitSync() .
See
 * @see inTransaction property.
 ...
 * @see FBStatement is derived form
 * @see FBResult class.
So it can fetch rows just like
 * @see FBresult object after call to
 * @see execSync, exec methods.
 */
declare class FBStatement mixins FBResult {

/**
 * Synchronously executes prepared statement with given parameters.
 * @desc You may fetch rows with methods inherited from
 * @see FBResult.
...
You may fetch rows with methods inherited from
 * @see FBResult after 'result' event emitted.
...
_read(
buffer: Buffer,
callback: (err: Error | null, buffer: Buffer, len: number) => void): void,
 ...
 */
declare class Stream mixins stream.Stream {
constructor(blob: FBBlob): this;
readable: boolean;
pause(): this;
resume(): this;
writable: boolean;
