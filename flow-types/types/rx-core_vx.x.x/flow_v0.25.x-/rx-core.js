...
declare interface Rx$Observable<T> {
  ...
  onErrorResumeNext(
    second: Rx$IPromise<T> | Rx$Observable<T>
  ): Rx$Observable<T>;
  ...
  groupBy<TKey, TElement>(
    ...
    keySerializer?: (key: TKey) => string
  ): Rx$Observable<Rx$GroupedObservable<TKey, T>>;
  groupBy<TKey, TElement>(
    keySelector: (value: T) => TKey,
    elementSelector: (value: T) => TElement,
    keySerializer?: (key: TKey) => string
  ): Rx$Observable<Rx$GroupedObservable<TKey, TElement>>;
  groupByUntil<TKey, TDuration>(
    keySelector: (value: T) => TKey,
    skipElementSelector: boolean,
    durationSelector: (
      group: Rx$GroupedObservable<TKey, T>
    ) => Rx$Observable<TDuration>,
    keySerializer?: (key: TKey) => string
  ): Rx$Observable<Rx$GroupedObservable<TKey, T>>;
  groupByUntil<TKey, TElement, TDuration>(
    ...
    elementSelector: (value: T) => TElement,
    durationSelector: (
      group: Rx$GroupedObservable<TKey, TElement>
    ) => Rx$Observable<TDuration>,
    keySerializer?: (key: TKey) => string
  ): Rx$Observable<Rx$GroupedObservable<TKey, TElement>>;
}
