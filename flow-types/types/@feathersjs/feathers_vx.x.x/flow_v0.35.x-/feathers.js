...
hook: HookContext) => (Promise<HookContext | SkipSymbol | void> | HookContext | SkipSymbol | void);
	declare export type SkipSymbol = Symbol | "__feathersSkipHooks";
	declare export interface HookContext<T> {
...
create(data: $Shape<T> | Array<$Shape<T>>, params?: Params): Promise<T | T[]>,
...
hooks(hooks: $Shape<HooksObject>): this
	...
	declare export type Service<T> = ServiceOverloads<T> & ServiceAddons<T> & ServiceMethods<T>;
	declare export type Application<ServiceTypes> = {
get(name: string): any,
...
service<L: $Keys<ServiceTypes>>(location: L): Service<$ElementType<ServiceTypes, L>>,
