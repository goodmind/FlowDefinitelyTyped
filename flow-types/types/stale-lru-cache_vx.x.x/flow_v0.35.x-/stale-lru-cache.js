...
get(key: K): V | void;
set(key: K, value: V, options?: string | Cache$Cache$SetOptions<K, V>): boolean;
has(key: K): boolean;
...
callback: Cache$Cache$OptionsCallback<K, V>): void
}
	declare type Cache$OptionsCallback<K, V> = (error: any, value?: V, options?: string | Cache$SetOptions<K, V>) => void;

declare type Cache$RevalidationCallback<K, V> = (key: K, callback: Cache$OptionsCallback<K, V>) => void;
