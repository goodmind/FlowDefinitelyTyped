declare interface LazyJS$LazyStatic {
  ...
  <T>(value: any): LazyJS$ObjectLikeSequence<T>;
  ...
  range(
    ...
    to: number,
    step?: number
  ): LazyJS$GeneratedSequence<number>;
...
declare interface LazyJS$SequenceBaser<T> {
  ...
  dropWhile(
    predicateFn: LazyJS$TestCallback<T, string | number>
  ): LazyJS$Sequence<T>;
  ...
  sort(
    sortFn?: LazyJS$CompareCallback,
    descending?: boolean
  ): LazyJS$Sequence<T>;
  sortBy(
    sortFn: LazyJS$NumberCallback<T>,
    descending?: boolean
  ): LazyJS$Sequence<T>;
