 ...
 */
server: NodeIPC$Server
}

declare type NodeIPC$EmptyCallback = () => void;

declare interface NodeIPC$Client {
 ...
 */
on(event: string, callback: (...args: any[]) => void): NodeIPC$Client,

/**
 * triggered when an error has occured
 */
on(event: "error", callback: (err: any) => void): NodeIPC$Client,
...
callback: (socket: Socket, destroyedSocketID: string) => void): NodeIPC$Client,

/**
 * triggered when ipc.config.rawBuffer is true and a message is received
 */
on(event: "data", callback: (buffer: Buffer) => void): NodeIPC$Client,
emit(event: string, value?: any): NodeIPC$Client,

/**
 * Unbind subscribed events
 */
off(event: string, handler: any): NodeIPC$Client
} 

declare type NodeIPC$Server = {
 ...
 */
stop(): void,
emit(value: any): NodeIPC$Client,
emit(event: string, value: any): NodeIPC$Client,
emit(
socket: Socket | NodeIPC$SocketConfig,
event: string,
value?: any): NodeIPC$Server,
emit(socketConfig: Socket | NodeIPC$SocketConfig, value?: any): NodeIPC$Server
} & NodeIPC$Client
...
declare interface NodeIPC$Config {
