...
| MemoType$MemoType$Return;
	...
	declare export class Memo<T: MemoType>  {
...
declare interface Operation$BaseOperation<T: OperationType> {
type: T,
source?: string
} 
...
declare function Operation$fromXDRObject<T: Operation>(xdrOperation: xdr$Operation<T>): T
...
declare function StrKey$decodeSha256Hash(data: string): Buffer
	...
	declare export class Transaction<TMemo: xdr$Memo, TOps: Operation[]>  {
...
memo?: xdr$Memo
	...
	declare class xdrHidden$Operation2<T: Operation> mixins xdr$xdr$XDRStruct {
...
static fromXDR(xdr: Buffer): xdr$TransactionResult
	...
	declare export function sign(data: Buffer, rawSecret: Buffer): Buffer

	declare export function verify(data: Buffer, signature: Buffer, rawPublicKey: Buffer): boolean
