...
declare module "commangular" {
  declare interface ICommAngularStatic {
     ...
     * @param commandName
     * It's the name of the command you are creating. It's useful to reference the command from the command provider.
     * @param commandFunction
     * It's the command class that will be executed when commangular runs this command.
     * It has to be something that implements ICommand. Same as angular syntax
     * @param commandConfig
     * It's and object with paramaters to configure the command execution.
     */
    create(
      commandName: string,
     ...
     * The interceptor descriptor has two parts 'Where' and 'What'.
     * Where do you want to intercept? you've 5 options :
     * -
     * @Before
     * : The interceptor will be executed before the command. You will be able to
     ...
     * -
     * @After
     * : The interceptor will be executed just after the command and before any other next
     * command. You can get the lastResult from the command, cancel execution etc etc.
     * -
     * @AfterExecution
     * : This intercetor is executed just after the command execute method and
     * it can get the result from the command and update it before the onResult method is executed.
     * -
     * @AfterThrowing
     * : This interceptor will be executed if the command or any interceptor of
     * the command throws an exception. You can get the error throwed injected to do what you need.
     * -
     * @Around
     * : The interceptor is executed around a command.That means that a especial
     ...
     * @param aspectFunction
     * It's the command class execute function that will be run for the given aspect.
     * @param order
     * You can chain any number of interceptors to the same command, so if you need to executed
  ...
  declare type IResultCommand = {
     ...
     * @param result
     * Value/object returned by the execution.
     */
     ...
     * @param error
     * The error that occured during execution
     */
  ...
  declare interface ICommAngularProvider {
     ...
     * @param eventName
     * An event that will be watched by commangular
     */
  ...
  declare interface ICommAngularService {
     ...
     * see http://commangular.org/docs/#dispatching-events
     * @param eventName
     * Name of the even that will trigger a command sequence
     * @param data
     * Data of any type that will be passed to the command.
     */
  ...
  declare interface ICommAngularDescriptor {
     ...
     * @param command
     * The name that was used to create the command.
     */
     ...
     * @param descriptor
     * Another descriptor attached to a sequnce of commands.
     */
     ...
     * see http://commangular.org/docs/#building-command-flows
     * @param expression
     * A string form expression that can make use of services to validate conditions.
     * @param services
     * A comma seperated list of services that are used in the above expression
     */
