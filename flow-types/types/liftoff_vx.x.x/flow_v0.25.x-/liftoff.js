declare module "liftoff" {
  import type { Extensions, Extension, ExtensionDescriptor } from "interpret";

  import type { PathSpec } from "fined";
  ...
  declare class Liftoff mixins EventEmitter {
    ...
    launch(
      ...
      callback: (env: Liftoff$Liftoff$LiftoffEnv) => void
    ): void;
    addListener(
      event: "require",
    ...
    addListener(
      event: "requireFail",
      listener: (name: string, err: any) => void
    ): this;
    addListener(
      event: "respawn",
  ...
  declare interface Liftoff$LaunchOptions {
    ...
    forcedFlags?:
      | string
      | string[]
      | ((env: Liftoff$LiftoffEnv) => string | string[]);
    completion?: string;
  }
