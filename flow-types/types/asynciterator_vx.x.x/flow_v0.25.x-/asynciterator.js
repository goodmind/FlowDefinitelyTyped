declare module "asynciterator" {
  ...
  declare export class AsyncIterator<T> mixins NodeJS.EventEmitter {
    ...
    transform<T2>(
      options?: SimpleTransformIteratorOptions<T, T2>
    ): SimpleTransformIterator<T, T2>;
    map<T2>(
      mapper: (item: T) => T2,
      self?: { [key: string]: any }
    ): SimpleTransformIterator<T, T2>;
  ...
  declare export class TransformIterator<S, T> mixins BufferedIterator<T> {
    ...
    _closeWhenDone(): void;
    constructor(
      source?: AsyncIterator<S> | TransformIteratorOptions<S>,
  ...
  declare export class SimpleTransformIterator<S, T>
    mixins TransformIterator<S, T> {
    _offset: number;
    _limit: number;
  ...
  declare export class MultiTransformIterator<S, T>
    mixins TransformIterator<S, T> {
    _transformerQueue: S[];
