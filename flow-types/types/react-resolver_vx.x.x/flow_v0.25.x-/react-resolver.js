declare module "react-resolver" {
  import type { ComponentType, StatelessComponent, Factory } from "react";
  ...
  declare export function resolve<OwnProps, K: string, V, MoreProps>(
    ...
    component: ComponentType<OwnProps & $ObjMapi<{ [k: K]: any }, <C>(C) => V>>
  ) => StatelessComponent<OwnProps & MoreProps>;

  declare export function resolve<OwnProps, ResolvableProps, MoreProps>(
    resolversMap: $ObjMapi<
      ResolvableProps,
      <K>(
        K
      ) => ResolveFn<OwnProps & MoreProps, $ElementType<ResolvableProps, K>>
    >
  ): (
    component: ComponentType<
      OwnProps &
        $ObjMapi<ResolvableProps, <K>(K) => $ElementType<ResolvableProps, K>>
    >
  ) => StatelessComponent<OwnProps & MoreProps>;
}
