...
declare type ArangoDB$HashAlgorithm = "sha512"
| "sha384"
| "sha256"
| "sha224"
| "sha1"
...
declare type ArangoDB$HttpMethod = "HEAD"
| "GET"
| "POST"
| "PUT"
| "PATCH"
...
declare type ArangoDB$HttpStatus = "continue"
| "switching protocols"
| "processing"
| "ok"
| "created"
...
declare type ArangoDB$ErrorName = "ERROR_NO_ERROR"
| "ERROR_FAILED"
| "ERROR_SYS_ERROR"
| "ERROR_OUT_OF_MEMORY"
| "ERROR_INTERNAL"
...
declare type ArangoDB$CollectionType = ArangoDB$DocumentCollectionType | ArangoDB$EdgeCollectionType;
...
declare interface ArangoDB$Index<T: {[key: string]: any}> {
id: string,
type: ArangoDB$IndexType,
...
declare type ArangoDB$DocumentLike = ArangoDB$ObjectWithId | ArangoDB$ObjectWithKey;

declare type ArangoDB$Patch<T> = $ObjMapi<T, <K>(K) => $ElementType<T, K> | ArangoDB$Patch<$ElementType<T, K>>>;
...
declare type ArangoDB$Document<T: {[key: string]: any}> = $ObjMapi<T, <K>(K) => $ElementType<T, K>> & ArangoDB$DocumentMetadata & {
_from?: string,
_to?: string
} & {
[key: string]: any
...
declare type ArangoDB$DocumentData<T: {[key: string]: any}> = $ObjMapi<T, <K>(K) => $ElementType<T, K>> & $Shape<ArangoDB$DocumentMetadata>;

declare type ArangoDB$Edge<T: {[key: string]: any}> = ArangoDB$Document<T> & {
_from: string,
_to: string
};

declare type ArangoDB$InsertResult<T: {[key: string]: any}> = {
new?: ArangoDB$Document<T>
} & ArangoDB$DocumentMetadata


declare type ArangoDB$UpdateResult<T: {[key: string]: any}> = {
old?: ArangoDB$Document<T>,
new?: ArangoDB$Document<T>
} & ArangoDB$UpdateMetadata


declare type ArangoDB$RemoveResult<T: {[key: string]: any}> = {
old?: ArangoDB$Document<T>
} & ArangoDB$DocumentMetadata
...
declare type ArangoDB$DocumentIterator<T: {[key: string]: any}> = (document: ArangoDB$Document<T>, number: number) => void;

declare interface ArangoDB$Collection<T: {[key: string]: any}> {
name(): string,
checksum(withRevisions?: boolean, withData?: boolean): ArangoDB$CollectionChecksum,
count(): number,
drop(options?: {
isSystem?: boolean
}): void,
...
path(): string,
properties(
properties?: ArangoDB$CollectionPropertiesOptions): ArangoDB$CollectionProperties,
revision(): string,
rotate(): void,
...
byExample(example: $Shape<ArangoDB$Document<T>>): ArangoDB$Cursor<ArangoDB$Document<T>>,
document(selector: string | ArangoDB$DocumentLike): ArangoDB$Document<T>,
document(
selectors: $ReadOnlyArray<string | ArangoDB$DocumentLike>): Array<ArangoDB$Document<T>>,
exists(name: string): boolean,
firstExample(example: $Shape<ArangoDB$Document<T>>): ArangoDB$Document<T> | null,
insert(
data: ArangoDB$DocumentData<T>,
options?: ArangoDB$InsertOptions): ArangoDB$InsertResult<T>,
insert(
array: $ReadOnlyArray<ArangoDB$DocumentData<T>>,
options?: ArangoDB$InsertOptions): Array<ArangoDB$InsertResult<T>>,
insert(
from: string,
to: string,
data: ArangoDB$DocumentData<T>,
options?: ArangoDB$InsertOptions): ArangoDB$InsertResult<T>,
edges(
vertex: string | ArangoDB$ObjectWithId | $ReadOnlyArray<string | ArangoDB$ObjectWithId>): Array<ArangoDB$Edge<T>>,
inEdges(
vertex: string | ArangoDB$ObjectWithId | $ReadOnlyArray<string | ArangoDB$ObjectWithId>): Array<ArangoDB$Edge<T>>,
outEdges(
vertex: string | ArangoDB$ObjectWithId | $ReadOnlyArray<string | ArangoDB$ObjectWithId>): Array<ArangoDB$Edge<T>>,
...
options?: ArangoDB$IterateOptions): void,
remove(
selector: string | ArangoDB$DocumentLike,
options?: ArangoDB$RemoveOptions): ArangoDB$RemoveResult,
remove(
...
removeByExample(
example: $Shape<ArangoDB$Document<T>>,
options?: ArangoDB$RemoveByExampleOptions): number,
rename(newName: string): void,
replace(
selector: string | ArangoDB$DocumentLike,
data: ArangoDB$DocumentData<T>,
options?: ArangoDB$ReplaceOptions): ArangoDB$UpdateResult<T>,
replace(
selectors: $ReadOnlyArray<string | ArangoDB$DocumentLike>,
data: $ReadOnlyArray<ArangoDB$DocumentData<T>>,
options?: ArangoDB$ReplaceOptions): Array<ArangoDB$UpdateResult<T>>,
replaceByExample(
example: $Shape<ArangoDB$Document<T>>,
...
options?: ArangoDB$UpdateOptions): ArangoDB$UpdateResult<T>,
update(
selectors: $ReadOnlyArray<string | ArangoDB$DocumentLike>,
data: $ReadOnlyArray<ArangoDB$Patch<ArangoDB$Document<T>>>,
options?: ArangoDB$UpdateOptions): Array<ArangoDB$UpdateResult<T>>,
updateByExample(
example: $Shape<ArangoDB$Document<T>>,
...
example: $Shape<ArangoDB$Document<T>>,
newValue: ArangoDB$Patch<ArangoDB$Document<T>>,
options?: ArangoDB$UpdateByExampleOptions): number
} 
...
declare interface ArangoDB$Cursor<T> {
toArray(): T[],
hasNext(): boolean,
next(): T,
...
declare interface ArangoDB$Statement<T> {
...
rename(newName: string): void,
properties(
newProperties?: ArangoDB$ArangoSearchViewPropertiesOptions): ArangoDB$ArangoSearchViewProperties
} 

declare type ArangoDB$ArangoSearchViewConsolidationType = "bytes" | "bytes_accum" | "count" | "fill";
...
_createDatabase(name: string, options?: empty, users?: ArangoDB$DatabaseUser[]): true,
_databases(): string[],
...
_create(
name: string,
properties?: ArangoDB$CreateCollectionOptions): ArangoDB$Collection,
...
bindVars?: {[key: string]: any},
options?: ArangoDB$QueryOptions): ArangoDB$Cursor,
_explain(query: ArangoDB$Query | string): void,
_parse(query: string): ArangoDB$ParsedQuery,
_document(name: string): ArangoDB$Document,
_exists(selector: string | ArangoDB$ObjectWithId): ArangoDB$DocumentMetadata,
_remove(selector: string | ArangoDB$ObjectWithId): ArangoDB$DocumentMetadata,
_replace(
selector: string | ArangoDB$ObjectWithId,
data: {[key: string]: any}): ArangoDB$DocumentMetadata,
_update(
selector: string | ArangoDB$ObjectWithId,
data: {[key: string]: any}): ArangoDB$DocumentMetadata,
...
name: string,
type: ArangoDB$ViewType,
properties: ArangoDB$ArangoSearchViewPropertiesOptions): ArangoDB$ArangoSearchView,
...
declare type Foxx$SimpleMiddleware = (req: Foxx$Request, res: Foxx$Response, next: Foxx$NextFunction) => void;

declare interface Foxx$DelegateMiddleware {
register: (endpoint: Foxx$Endpoint) => Foxx$SimpleMiddleware
} 

declare type Foxx$Middleware = Foxx$SimpleMiddleware | Foxx$DelegateMiddleware;
...
fromClient?: (body: string | Buffer, req: Foxx$Request, type: Foxx$MediaType) => any,
forClient?: (body: any) => {
data: string,
headers: {
...
declare type Foxx$ConfigurationType = "integer"
| "boolean"
| "string"
| "number"
| "json"
...
mount: string,
collection(name: string): ArangoDB$ArangoDB$Collection | null,
collectionName(name: string): string,
createDocumentationRouter(
opts?: $Shape<Foxx$DocumentationRouterOptions> | $ElementType<Foxx$DocumentationRouterOptions, "before"> | $ElementType<Foxx$DocumentationRouterOptions, "swaggerRoot">): Foxx$Router,
...
hostname: string,
method: ArangoDB$ArangoDB$HttpMethod,
originalUrl: string,
path: string,
pathParams: {
[key: string]: any
...
options?: {
secret?: string,
algorithm?: ArangoDB$ArangoDB$HashAlgorithm
}): string | null,
...
is(...types: string[]): string,
json(): any,
makeAbsolute(path: string, query?: string | {
[key: string]: string | void
}): string,
...
json(data: any): this,
redirect(status: number | ArangoDB$ArangoDB$HttpStatus, path: string): this,
...
sendStatus(status: number | ArangoDB$ArangoDB$HttpStatus): this,
setHeader(name: string, value: string): this,
set(name: string, value: string): this,
set(headers: {
[name: string]: string
}): this,
status(status: number | ArangoDB$ArangoDB$HttpStatus): this,
throw(
status: number | ArangoDB$ArangoDB$HttpStatus,
reason: string,
error: Error): empty,
throw(
status: number | ArangoDB$ArangoDB$HttpStatus,
...
throw(status: number | ArangoDB$ArangoDB$HttpStatus, error: Error): empty,
throw(
status: number | ArangoDB$ArangoDB$HttpStatus,
options?: {
cause?: Error,
...
mimes: string[],
description?: string): this,
response(status: number | ArangoDB$ArangoDB$HttpStatus, description: string): this,
summary(summary: string): this,
description(description: string): this,
deprecated(deprecated: boolean): this,
error(status: number | ArangoDB$ArangoDB$HttpStatus, description: string): this,
tag(...tags: string[]): this
} 

declare function Foxx$route(handler: Foxx$Handler, name?: string): Foxx$Endpoint
...
name?: string): Foxx$Endpoint
	...
	declare module '@arangodb' {
        declare function aql(strings: TemplateStringsArray, ...args: any[]): ArangoDB$ArangoDB$Query
...
declare function aql$literal(value: any): ArangoDB$ArangoDB$AqlLiteral

	declare function query(strings: TemplateStringsArray, ...args: any[]): ArangoDB$ArangoDB$Cursor

	declare function time(): number

	declare var db: ArangoDB$ArangoDB$Database & {
[key: string]: ArangoDB$ArangoDB$Collection | void
};
	declare var errors: $ObjMapi<{[k: ArangoDB$ArangoDB$ErrorName]: any}, <Name>(Name) => {
code: number,
message: string
}>;
    }
	...
	declare module '@arangodb/foxx/router' {
        declare function createRouter(): Foxx$Foxx$Router
...
mount: string
	...
	declare type JobCallback = (result: any, jobData: any, job: ArangoDB$ArangoDB$Document<Job>) => void;
...
get(jobId: string): ArangoDB$ArangoDB$Document<Job>,
...
all(script?: Script): string[]
	...
	declare export {
          create,delete,get,JobOptions,Job,Queue,QueueItem,Script
        }
...
graphql?: GraphQLModule
	...
	declare function createGraphQLRouter(options: GraphQLOptions | GraphQLSchema): Foxx$Foxx$Router

	declare module.exports: typeof createGraphQLRouter

    }

	declare module '@arangodb/foxx/sessions' {
        declare interface SessionsOptions {
storage: Foxx$Foxx$SessionStorage | string | ArangoDB$ArangoDB$Collection,
transport: Foxx$Foxx$SessionTransport | Foxx$Foxx$SessionTransport[] | "cookie" | "header",
autoCreate?: boolean
} 
	declare function sessionsMiddleware(options: SessionsOptions): Foxx$Foxx$SessionsMiddleware

	declare module.exports: typeof sessionsMiddleware

    }

	declare module '@arangodb/foxx/sessions/storages/collection' {
        declare interface CollectionStorageOptions {
collection: string | ArangoDB$ArangoDB$Collection,
ttl?: number,
pruneExpired?: boolean,
autoUpdate?: boolean
} 
	declare function collectionStorage(
options: CollectionStorageOptions | $ElementType<CollectionStorageOptions, "collection">): Foxx$Foxx$CollectionSessionStorage

	declare module.exports: typeof collectionStorage

    }

	declare module '@arangodb/foxx/sessions/storages/jwt' {
        declare interface SafeJwtStorageOptions {
algorithm?: ArangoDB$ArangoDB$JwtAlgorithm,
secret: string,
ttl?: number,
verify?: boolean,
maxExp?: number
...
options: SafeJwtStorageOptions | UnsafeJwtStorageOptions | $ElementType<SafeJwtStorageOptions, "secret">): Foxx$Foxx$SessionStorage

	declare module.exports: typeof jwtStorage
...
httpOnly?: string
	...
	declare function cookieTransport(options?: CookieTransportOptions): Foxx$Foxx$SessionTransport

	declare function cookieTransport(name: string): Foxx$Foxx$SessionTransport

	declare module.exports: typeof cookieTransport
...
name?: string
	...
	declare function headerTransport(options?: HeaderTransportOptions): Foxx$Foxx$SessionTransport

	declare function headerTransport(name: string): Foxx$Foxx$SessionTransport

	declare module.exports: typeof headerTransport
...
method?: ArangoDB$ArangoDB$HashAlgorithm,
saltLength?: number
} 
	declare function createAuth(options?: AuthOptions): Authenticator

	declare module.exports: typeof createAuth
...
fetchRequestToken(oauth_callback: string, qs?: {
[key: string]: string | void
}): any,
getAuthUrl(oauth_token: string, qs?: {
[key: string]: string | void
}): string,
...
authorization: string
	...
	declare module.exports: typeof createOAuth1Client
...
getAuthUrl(redirect_uri: string, options?: {
response_type?: string
}): string,
exchangeGrantToken(code: string, redirect_uri: string, options?: {
grant_type?: string
}): any,
fetchActiveUser(access_token: string): any
	...
	declare module.exports: typeof createOAuth2Client
...
returnBodyOnError?: boolean
	...
	declare function method(options: {
url: string
} & RequestOptions): Foxx$Response
...
key: string,
message: string,
algorithm: ArangoDB$ArangoDB$JwtAlgorithm): string

	declare function jwtEncode(key: null, message: string, algorithm: "none"): string

	declare function jwtDecode(
key: string | null,
token: string,
noVerify?: boolean): {[key: string]: any} | null
	...
	declare function pbkdf2(salt: string, password: string, iterations: number, keyLength: number): string

	declare function hmac(
key: string,
message: string,
algorithm: ArangoDB$ArangoDB$HashAlgorithm): string
...
[key: string]: number | void
	...
	declare interface Path<A: {[key: string]: any}, B: {[key: string]: any}, E: {[key: string]: any}, V: {[key: string]: any}> {
source: ArangoDB$ArangoDB$Document<A>,
destination: ArangoDB$ArangoDB$Document<B>,
edges: Array<ArangoDB$ArangoDB$Edge<E>>,
vertice: Array<ArangoDB$ArangoDB$Document<A | B | V>>
} 
	declare interface ShortestPath<T: {[key: string]: any}> {
vertices: string[],
edges: Array<ArangoDB$ArangoDB$Edge<T>>,
distance: number
} 
...
[key: string]: number | void
	...
	declare type Example = Array<{[key: string]: any} | string> | {[key: string]: any} | string | null;
...
direction?: ArangoDB$ArangoDB$EdgeDirection,
edgeExamples?: Example,
...
options: ConnectingEdgesOptions): ArangoDB$ArangoDB$Edge,
_fromVertex(edgeId: string): ArangoDB$ArangoDB$Document,
_toVertex(edgeId: string): ArangoDB$ArangoDB$Document,
...
vertex2Options?: NeighborsOptions): CommonNeighbors[],
_countCommonNeighbors(
vertex1Example: Example,
vertex2Example: Example,
vertex1Options?: NeighborsOptions,
vertex2Options?: NeighborsOptions): CountCommonNeighbors[],
_commonProperties(
vertexExample1: Example,
vertex2Example: Example,
options?: CommonPropertiesOptions): CommonProperties[],
_countCommonProperties(
vertex1Example: Example,
vertex2Example: Example,
options?: CommonPropertiesOptions): CountCommonProperties[],
_paths(options?: PathsOptions): Path[],
_shortestPath(
startVertexExample: Example,
endVertexExample: Example,
options?: ShortestPathOptions): ShortestPath[],
_distanceTo(
startVertexExample: Example,
endVertexExample: Example,
options?: ShortestPathOptions): Distance[],
...
orphanCollections?: string[]): Graph & {
[key: string]: ArangoDB$ArangoDB$Collection | void
}

	declare function _list(): string[]

	declare function _graph(name: string): Graph & {
[key: string]: ArangoDB$ArangoDB$Collection | void
}
...
toVertexCollections: string[] | string): EdgeDefinition

	declare function _edgeDefinitions(...relations: EdgeDefinition[]): EdgeDefinition[]

	declare function _extendEdgeDefinitions(
edgeDefinitions: EdgeDefinition[],
...relations: EdgeDefinition[]): EdgeDefinition[]

    }

	declare module '@arangodb/locals' {
        declare var context: Foxx$Foxx$Context;
    }

	declare interface NodeModule {
context: Foxx$Foxx$Context
} 
