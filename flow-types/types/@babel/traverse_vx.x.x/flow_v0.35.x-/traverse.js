...
checkBlockScopedCollisions(local: Node, kind: string, name: string, id: {[key: string]: any}): void;
...
constantViolations: NodePath[]
	...
	declare export type Visitor<S> = VisitNodeObject<S, Node> & $ObjMapi<{[k: $ElementType<Node, "type">]: any}, <Type>(Type) => VisitNode<S, Extract<Node, {
type: Type
}>>> & $ObjMapi<t.Aliases, <K>(K) => VisitNode<S, $ElementType<t.Aliases, K>>>;
	declare export type VisitNode<S, P> = VisitNodeFunction<S, P> | VisitNodeObject<S, P>;
	declare export type VisitNodeFunction<S, P> = (path: NodePath<P>, state: S) => void;
...
buildCodeFrameError<TError: Error>(msg: string, Error?: (msg: string) => TError): TError;
...
get<K: $Keys<T>>(
key: K,
context?: boolean | TraversalContext): "There was Conditional Type, use $Call utility type";
