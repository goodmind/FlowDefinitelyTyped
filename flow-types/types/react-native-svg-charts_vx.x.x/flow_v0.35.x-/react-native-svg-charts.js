declare module 'react-native-svg-charts' {
        import type {
          ScaleBand,ScaleLinear,ScaleLogarithmic,ScalePower,ScaleTime
        } from 'd3-scale';
	...
	import type {
          CommonPathProps,LinearGradientProps,LineProps,PathProps,RadialGradientProps,TextProps
        } from 'react-native-svg';

	declare export type ScaleType<Range, Output> = ScaleLinear<Range, Output> | ScaleLogarithmic<Range, Output> | ScalePower<Range, Output> | ScaleTime<Range, Output>;
	declare export type ScaleFunction = () => ScaleType<any, any> | ScaleBand<any>;
	declare export type AccessorFunction<T, U> = (props: {
item: T,
index: number
}) => U;
	declare export type SortFunction<T> = (a: T, b: T) => number;
	declare export type OffsetFunction = (series: Series<any, any>, order: number[]) => void;
...
valueAccessor?: AccessorFunction<T, number>
	...
	declare export class PieChart<T: PieChartData> mixins React.PureComponent<PieChartProps<T>> {}
	declare export type AreaChartProps<T> = {
start?: number
} & ChartProps<T>

	declare export class AreaChart<T> mixins React.PureComponent<AreaChartProps<T>> {}
...
renderDecorator?: () => {}
	...
	declare export class StackedAreaChart<T> mixins React.PureComponent<StackedAreaChartProps<T>> {
static extractDataPoints<T>(
data: T[],
keys: $ReadOnlyArray<$Keys<T>>,
order?: OrderFunction,
offset?: OffsetFunction): number[]
}
...
extra?: () => {}
	...
	declare export class StackedBarChart<T> mixins React.PureComponent<StackedBarChartProps<T>> {
static extractDataPoints<T>(
data: T,
keys: $ReadOnlyArray<$Keys<T>>,
order?: OrderFunction,
...
spacingOuter?: number
	...
	declare export class BarChart<T> mixins React.PureComponent<BarChartProps<T>> {}
...
animateDuration?: number
	...
	declare export class ProgressCircle mixins React.PureComponent<ProgressCircleProps> {}
...
pointStroke?: string
	...
	declare export class Decorators$HorizontalLine mixins React.Component<HorizontalLineProps> {}

declare export class Decorators$Point mixins React.Component<PointProps> {}

declare export class Decorators$Tooltip mixins React.Component<TooltipProps> {}
