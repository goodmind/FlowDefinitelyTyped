...
export interface uiGrid$IGridInstanceOf<TEntity> {
   ...
   * @param {uiGrid$IColumnDef} colDef The column definition
   ...
   * @param {uiGrid$IRowBuilder} rowBuilder function to be called
   */
 ...
 * @param {uiGrid$IBuildColumnsOptions} options An object containing options to use when building columns
 ...
 */
   ...
   * @param {boolean} refreshRows Refresh the rows?
   * @param {boolean} clearConditions Clear conditions?
   * @param {boolean} clearFlags Clear flags?
   ...
   * @param {uiGrid$IGridRow} row Row to access
   * @param {uiGrid$IGridColumn} col Column to access
   ...
   * @param {string} name Column name
   * @returns {uiGrid$IColumnDef} The column definition
   */
   ...
   * @returns {Array<uiGrid$IGridColumn>} All data columns
   */
   ...
   * @param {Array<uiGrid$IGridRow>} rows The rows to look in.  if not provided then it looks in grid.rows
   */
  getRow(
    rowEntity: TEntity,
    rows?: Array<uiGrid$IGridRowOf<TEntity>>
  ): uiGrid$IGridRowOf<TEntity>;
   ...
   * @param {uiGrid$IGridColumn} column The column
 ...
 */
   ...
   * @param {uiGrid$IGridRow} gridRow reference to gridRow
   * @returns {uiGrid$IGridRow} the gridRow with all additional behavior added
   */
  processRowBuilders(
 ...
 */
   ...
   * @param {uiGrid$IColumnBuilder} columnBuilder function to be called
   */
 ...
 * @param {uiGrid$IColumnProcessor} columnProcessor column processor function, which
 ...
 */
  registerColumnsProcessor(
    columnProcessor: uiGrid$IColumnProcessor<TEntity>,
    priority: number
  ): void;
 ...
 * @param {(grid: uiGrid$IGridInstance) => void} callback function to be called
 ...
 * modified.
 * @param {uiGrid$IRowProcessor} rowProcessor rows processor function, which
 ...
 */
  registerRowsProcessor(
    rowProcessor: uiGrid$IRowProcessor<TEntity>,
    priority: number
  ): void;
   ...
   * @param {uiGrid$IStyleComputation} styleComputation function
   */
   ...
   * @param {uiGrid$IRowProcessor} rows processor function
   */
  removeRowsProcessor(rows: uiGrid$IRowProcessor<TEntity>): void;

  /**
   * Return the columns that the grid is currently being sorted by
   * @param {uiGrid$IGridColumn} excludedColumn Optional GridColumn to exclude from having its sorting reset
   */
   ...
   * @param {uiGrid$IColumnDef} colDef to make visible
   ...
   * @param {uiGrid$IGridRow} gridRow row to make visible
   * @param {uiGrid$IGridColumn} gridCol column to make visible
   * @returns {ng.IPromise<any>} a promise that is resolved when scrolling is complete
   */
  scrollToIfNecessary(
    gridRow: uiGrid$IGridRowOf<TEntity>,
    gridCol: uiGrid$IGridColumnOf<TEntity>
  ): ng.IPromise<any>;
 ...
 * @param {uiGrid$IGridColumn} column Column to set the sorting on
 ...
 * @returns {ng.IPromise<uiGrid$IGridColumn>} A resolved promise that supplies the column.
 */
  sortColumn(
    column: uiGrid$IGridColumnOf<TEntity>,
    direction?: string,
    add?: boolean
  ): ng.IPromise<uiGrid$IGridColumnOf<TEntity>>;
...
export interface uiGrid$IColumnBuilder<TEntity> {
  (
    colDef: uiGrid$IColumnDefOf<TEntity>,
    col: uiGrid$IGridColumnOf<TEntity>,
    gridOptions: uiGrid$IGridOptionsOf<TEntity>
  ): void;
}
...
export interface uiGrid$IRowProcessor<TEntity> {
  (
    renderedRowsToProcess: Array<uiGrid$IGridRowOf<TEntity>>,
    columns: Array<uiGrid$IGridColumnOf<TEntity>>
  ): Array<uiGrid$IGridRowOf<TEntity>>;
}

export interface uiGrid$IColumnProcessor<TEntity> {
  (
    renderedColumnsToProcess: Array<uiGrid$IGridColumnOf<TEntity>>,
    rows: Array<uiGrid$IGridRowOf<TEntity>>
  ): Array<uiGrid$IGridColumnOf<TEntity>>;
}
...
export type uiGrid$IGridOptionsOf<TEntity> = {
   ...
   * @param {uiGrid$IGridRow} row The row for which you want the unique id
  ...
  fastWatch?: boolean
} & cellNav.uiGrid$IGridOptions &
  edit.uiGrid$IGridOptions &
  expandable.uiGrid$IGridOptions &
  exporter.uiGrid$IGridOptions<TEntity> &
  grouping.uiGrid$IGridOptions &
  importer.uiGrid$IGridOptions<TEntity> &
  infiniteScroll.uiGrid$IGridOptions &
  moveColumns.uiGrid$IGridOptions &
  pagination.uiGrid$IGridOptions &
  pinning.uiGrid$IGridOptions &
  resizeColumns.uiGrid$IGridOptions &
  rowEdit.uiGrid$IGridOptions &
  saveState.uiGrid$IGridOptions &
  selection.uiGrid$IGridOptions &
  treeBase.uiGrid$IGridOptions<TEntity> &
  treeView.uiGrid$IGridOptions;

export interface uiGrid$IGridCoreApi<TEntity> {
  /**
   * adds a row header column to the grid
   * @param {uiGrid$IColumnDef} column Column Definition
   * @param {number} order Number that indicates where the column should be placed in the grid.
   * @param {boolean} stopColumnBuild Prevents the buildColumn callback from being triggered. This is useful to improve performance of the grid during initial load.
   */
  addRowHeaderColumn(
    column: uiGrid$IColumnDefOf<TEntity>,
    order?: number,
    stopColumnBuild?: boolean
  ): void;
 ...
 * @param {uiGrid$IGridInstance} grid the grid on which we are acting
 * @param {Array<uiGrid$IMenuItem>} items menu items in the format as described in the tutorial, with
 ...
 */
  addToGridMenu(
    ...
    items: Array<uiGrid$IMenuItem>
  ): void;

  /**
   * Clears all filters and optionally refreshes the visible rows.
   * @param {boolean} refreshRows Defaults to true.
   * @param {boolean} clearConditions Defaults to false.
   * @param {boolean} clearFlags Defaults to false.
   * @returns {ng.IPromise<any>} If `refreshRows` is true, returns a promise of the rows refreshing.
   */
  clearAllFilters(
    refreshRows?: boolean,
    clearConditions?: boolean,
    clearFlags?: boolean
  ): ng.IPromise<Array<uiGrid$IGridRowOf<TEntity>>>;
   ...
   * @returns {Array<uiGrid$IGridRow>} an array of gridRow
   */
  getVisibleRows(): Array<uiGrid$IGridRowOf<TEntity>>;
 ...
 * modified.
 * @param {uiGrid$IColumnProcessor} processorFunction columns processor function, which
is run in the context of the grid (i.e. this for the function will be the grid), and must
 ...
 */
  registerColumnsProcessor(
    processorFunction: uiGrid$IColumnProcessor<TEntity>,
    priority: number
  ): void;
 ...
 */
  ...
  on: {
     ...
     * @param {uiGrid$canvasHeightChangedHandler} handler Callback
     */
    canvasHeightChanged: (
      scope: ng.IScope,
      handler: uiGrid$canvasHeightChangedHandler
    ) => void,
...
scope is destroyed
 * @param {uiGrid$columnVisibilityChangedHandler} callBack Will be called when the event is emited.
The function passes back the GridCol that has changed
 */
    columnVisibilityChanged: (
      scope: ng.IScope,
      callBack: uiGrid$columnVisibilityChangedHandler<TEntity>
    ) => void,
     ...
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$filterChangedHandler} handler Callback
     */
    filterChanged: (
      scope: ng.IScope,
      handler: uiGrid$filterChangedHandler<TEntity>
    ) => void,

    /**
     * is raised after the cache of visible rows is changed
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$rowsRenderedHandler} handler callback
     */
    rowsRendered: (
      scope: ng.IScope,
      handler: uiGrid$rowsRenderedHandler<TEntity>
    ) => void,
     ...
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$rowsVisibleChangedHandler} handler callback
     */
    rowsVisibleChanged: (
      scope: ng.IScope,
      handler: uiGrid$rowsVisibleChangedHandler<TEntity>
    ) => void,

    /**
     * is raised when scroll begins. Is throttled, so won't be raised too frequently
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$scrollBeginHandler} handler callback
     */
     ...
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$scrollEndHandler} handler callback
     */
     ...
     * @param {ng.IScope} scope Grid scope
     * @param {uiGrid$sortChangedHandler} handler callback
     */
    sortChanged: (
      scope: ng.IScope,
      handler: uiGrid$sortChangedHandler<TEntity>
    ) => void
  };
}
...
export interface uiGrid$columnVisibilityChangedHandler<TEntity> {
   ...
   * @param {uiGrid$IGridColumn} column Ui Grid column
   */
...
export interface uiGrid$filterChangedHandler<TEntity> {
   ...
   * @param {uiGrid$IGridApi} gridApi grid api
   */
...
export interface uiGrid$rowsRenderedHandler<TEntity> {
   ...
   * @param {uiGrid$IGridApi} gridApi Grid api object
   */
  (gridApi: uiGrid$IGridApiOf<TEntity>): void;
}
...
export interface uiGrid$sortChangedHandler<TEntity> {
   ...
   * @param {uiGrid$IGridInstance} grid instance
   * @param {uiGrid$IGridColumn} array of gridColumns that have sorting on them, sorted in priority order
   */
  (
    grid: uiGrid$IGridInstanceOf<TEntity>,
    columns: Array<uiGrid$IGridColumnOf<TEntity>>
...
declare module "cellNav" {
  ...
  declare export interface ICellNavApi<TEntity> {
     ...
     * @param {uiGrid$IColumnDef} colDef Column definition to make visible and set focus
     */
    scrollToFocus(
      rowEntity: TEntity,
  ...
  declare export interface IRowColConstructor<TEntity> {
    new(
      row: uiGrid$uiGrid$IGridRowOf<TEntity>,
      col: uiGrid$IGridColumnOf<TEntity>
    ): IRowCol<TEntity>;
  }
 ...
 * @example editFileChooserCallBack: (gridRow, gridCol, files) => {
 ...
 */
    editFileChooserCallback?: (
      gridRow: uiGrid$uiGrid$IGridRowOf<TEntity>,
      gridCol: uiGrid$IGridColumnOf<TEntity>,
      files: FileList
    ) => void;
 ...
 * @example colDef.field = 'state.name';
 ...
 */
  ...
  declare export interface afterCellEditHandler<TEntity> {
     ...
     * @param {uiGrid$IColumnDefOf} colDef The column that was edited
 ...
 */
  ...
  declare export interface cancelCellEditHandler<TEntity> {
    (rowEntity: TEntity, colDef: uiGrid$IColumnDefOf<TEntity>): void;
  }
...
declare module "expandable" {
  ...
  declare export interface rowExpandedStateChangedHandler<TEntity> {
    /**
     * Raised when cell editing is complete
     * @param {uiGrid$IGridRow} row The row that was expanded
     */
...
declare module "exporter" {
  ...
  declare export interface IGridOptions<TEntity> {
    ...
    /**
 * @deprecated DEPRECATED - exporterAllDataFn used to be called this, but it wasn't a promise,
 ...
 */
     ...
     * @param {uiGrid$IGridInstance} grid The grid
     * @param {uiGrid$IGridRow} row Current row
     * @param {uiGrid$IGridColumn} col Current column
 ...
 */
  ...
  declare export interface IGridGroupingConfiguration {
     ...
     */
    uiGrid$grouping: Array<IGripGroup>;
  ...
  declare export interface aggregationChangedHandler<TEntity> {
    /**
 * raised whenever aggregation is changed, added or removed from a column
 * @param {uiGrid$IGridColumn} col the column which on which aggregation changed.
 ...
 * @param {uiGrid$IGridColumn} col the column which on which grouping changed.
...
declare module "importer" {
  ...
  declare export interface IGridOptions<TEntity> {
     ...
     * @param {uiGrid$IGridInstance} grid The grid we're importing into, may be useful in some way
     * @param {Array<TEntity>} newObjects An array of new objects that you should add to your data
     */
    importerDataAddCallback?: (
      grid: uiGrid$IGridInstanceOf<TEntity>,
      newObjects: Array<TEntity>
    ) => void;
 ...
 * @param {uiGrid$IGridInstance} grid the grid we're importing into, may be useful if you're positioning messages
 ...
 * @param {uiGrid$IGridInstance} grid the grid we're importing into
 ...
 */
     ...
     * @param {uiGrid$IGridInstance} grid the grid we're working on
     * @param {boolean} scrollUp flag that there are pages upwards, fire infinite scroll events upward
     * @param {boolean} scrollDown flag that there are pages downwards, so fire infinite scroll events downward
     * @returns {ng.IPromise<any>} A promise that is resolved when scrolling finishes
     */
...
declare module "pinning" {
  ...
  declare export interface IGridPinningApi<TEntity> {
     ...
     * @param {uiGrid$IGridColumn} col The column being pinned
  ...
  declare export interface columnPinHandler {
     ...
     * @param {uiGrid$IColumnDef} colDef The column that was changed
...
declare module "resizeColumns" {
  ...
  declare export interface columnSizeChangedHandler {
     ...
     * @param {uiGrid$IColumnDef} colDef The column that was resized
 ...
 */
     ...
     * @param {uiGrid$IGridInstance} grid The target grid
     * @returns {Array<uiGrid$IGridRow>} An array of gridRows that are currently dirty
     */
    getDirtyRows(
      grid?: uiGrid$IGridInstanceOf<TEntity>
    ): Array<uiGrid$IGridRowOf<TEntity>>;
     ...
     * @param {uiGrid$IGridInstance} grid The target grid
     * @returns {Array<uiGrid$IGridRow>} An array of gridRows that are currently in error
     */
    getErrorRows(
      grid?: uiGrid$IGridInstanceOf<TEntity>
    ): Array<uiGrid$IGridRowOf<TEntity>>;
...
declare module "saveState" {
  ...
  declare export interface IGridSavedState {
    ...
    uiGrid$selection: Array<IRowVal>;
    uiGrid$grouping: uiGrid$grouping.IGridGroupingConfiguration;
    uiGrid$treeView: uiGrid$treeBase.ITreeState;
  }
...
declare module "selection" {
  ...
  declare export interface IGridOptions {
     ...
     * property.
     */
    isRowSelectable?: (row: uiGrid$IGridRow) => boolean;
  ...
  declare export interface IGridSelectionApi<TEntity> {
     ...
     * @returns {Array<uiGrid$IGridRow>} The selected rows
     */
    getSelectedGridRows(): Array<uiGrid$uiGrid$IGridRowOf<TEntity>>;
  ...
  declare export interface rowSelectionChangedHandler<TEntity> {
     ...
     * @param {uiGrid$IGridRow} row The selected rows
     * @param {ng.IAngularEvent} event object if raised from event
     */
    (row: uiGrid$uiGrid$IGridRowOf<TEntity>, event?: ng.IAngularEvent): void;
  }
  declare export interface rowSelectionChangedBatchHandler<TEntity> {
     ...
     */
    (
      row: Array<uiGrid$uiGrid$IGridRowOf<TEntity>>,
...
declare module "treeBase" {
  ...
  declare export interface IColumnDef<TEntity> {
     ...
     * @param {uiGrid$IGridRow} row Row objet
     */
  ...
  declare export interface IGridTreeBaseApi<TEntity> {
     ...
     * @param {uiGrid$IGridRow} row The row to collapse
     */
     ...
     * collapsed
     * @param {uiGrid$IGridRow} row The row to collapse children of
     */
     ...
     * @param {uiGrid$IGridRow} row The row to expand
     */
    expandRow(row: uiGrid$IGridRowOf<TEntity>): void;

    /**
     * Get the children of the specified row
     * @param {uiGrid$IGridRow} row The row you want the children of
 ...
 */
     ...
     * @param {uiGrid$IGridRow} row The row to toggle
     */
 ...
 * @param {uiGrid$IGridRow} row The row that was collapsed.  You can also retrieve the grid from this row with
row.grid
 */
    (row: uiGrid$IGridRowOf<TEntity>): void;
  }
 ...
 * @param {uiGrid$IGridRow} row The row that was expanded.  You can also retrieve the grid from this row with
row.grid
 */
    (row: uiGrid$IGridRowOf<TEntity>): void;
  }
 ...
 */
  suppressEvents(
    listenerFuncs: Function | Array<Function>,
    callBackFn: Function
  ): void;
   ...
   * Cell Nav Api
   */
  uiGrid$cellNav: uiGrid$cellNav.ICellNavApi<TEntity>;

  /**
   * Move Columns Api
   */
  colMovable: uiGrid$moveColumns.IGridMoveColumnsApi;

  /**
   * Grid row resizable Api
   */
  colResizable: uiGrid$resizeColumns.IGridResizeColumnsApi;

  /**
   * Edit Api
   */
  uiGrid$edit: uiGrid$edit.IGridEditApi<TEntity>;

  /**
   * Expandable Api
   */
  uiGrid$expandable: uiGrid$expandable.IGridExpandableApi<TEntity>;

  /**
   * Exporter Api
   */
  uiGrid$exporter: uiGrid$exporter.IGridExporterApi;

  /**
   * Grouping Api
   */
  uiGrid$grouping: uiGrid$grouping.IGridGroupingApi<TEntity>;

  /**
   * Importer Api
   */
  uiGrid$importer: uiGrid$importer.IGridImporterApi;

  /**
   * Infinite Scroll Api
   */
  uiGrid$infiniteScroll: uiGrid$infiniteScroll.IGridInfiniteScrollApi<TEntity>;

  /**
   * Pagination Api
   */
  uiGrid$pagination: uiGrid$pagination.IGridPaginationApi;

  /**
   * Pinning Api
   */
  uiGrid$pinning: uiGrid$pinning.IGridPinningApi<TEntity>;

  /**
   * Grid Row Edit Api
   */
  uiGrid$rowEdit: uiGrid$rowEdit.IGridRowEditApi<TEntity>;

  /**
   * Grid Save State Api
   */
  uiGrid$saveState: uiGrid$saveState.IGridSaveStateApi;

  /**
   * Selection Api
   */
  uiGrid$selection: uiGrid$selection.IGridSelectionApi<TEntity>;

  /**
   * Tree Base Api
   */
  uiGrid$treeBase: uiGrid$treeBase.IGridTreeBaseApi<TEntity>;
...
export interface uiGrid$IGridRowConstructor<TEntity> {
  ...
  new(
    entity: TEntity,
    index: number,
    reference: uiGrid$IGridInstanceOf<TEntity>
  ): uiGrid$IGridRowOf<TEntity>;
}

export type uiGrid$IGridRow = uiGrid$IGridRowOf<any>;
...
} & cellNav.uiGrid$IGridRow &
  edit.uiGrid$IGridRow &
  exporter.uiGrid$IGridRow &
  selection.uiGrid$IGridRow &
  expandable.uiGrid$IGridRow;
...
export interface uiGrid$IGridColumnOf<TEntity> {
   ...
   * Column definition
   */
  colDef: uiGrid$uiGrid$IColumnDefOf<TEntity>;

  /**
   * Default sort on this column
   */
  defaultSort?: uiGrid$ISortInfo;
 ...
 */
export type uiGrid$IColumnDef = uiGrid$IColumnDefOf<any>;
 ...
 */
   ...
   */
  width?: number | string
} & cellNav.uiGrid$IColumnDef &
  edit.uiGrid$IColumnDef<TEntity> &
  exporter.uiGrid$IColumnDef &
  grouping.uiGrid$IColumnDef &
  moveColumns.uiGrid$IColumnDef &
  pinning.uiGrid$IColumnDef &
  resizeColumns.uiGrid$IColumnDef &
  treeBase.uiGrid$IColumnDef<TEntity>;
...
declare module "ui-grid" {
  ...
  declare export default typeof uiGrid;
}
