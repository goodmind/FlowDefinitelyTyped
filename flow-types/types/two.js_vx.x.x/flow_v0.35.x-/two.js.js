...
makeRectangle(x: number, y: number, width: number, height: number): Two$Two$Rectangle;
makeRoundedRectangle(
x: number,
y: number,
width: number,
height: number,
radius: number): Two$Two$RoundedRectangle;
makeCircle(x: number, y: number, radius: number): Two$Two$Circle;
makeEllipse(x: number, y: number, width: number, height: number): Two$Two$Ellipse;
makeStar(ox: number, oy: number, or: number, ir: number, sides: number): Two$Two$Star;
makePolygon(x: number, y: number, radius: number, sides: number): Two$Two$Polygon;
makeArcSegment(
ox: number,
oy: number,
ir: number,
...
sa: number,
ea: number,
res?: number): Two$Two$ArcSegment;
makeCurve(...coords: Array<number | boolean>): Two$Two$Path;
makeCurve(points: $ReadOnlyArray<Two$Two$Vector>, open?: boolean): Two$Two$Path;
makePath(...coords: Array<number | boolean>): Two$Two$Path;
makePath(points: $ReadOnlyArray<Two$Two$Vector>, open?: boolean): Two$Two$Path;
...
constructor(vertices: $ReadOnlyArray<Two$Vector>, closed: boolean, curved: boolean, manual?: boolean): this;
id: number;
stroke: Two$Color;
fill: Two$Color;
linewidth: number;
...
constructor(ox: number, oy: number, or: number, ir?: number, sides?: number): this
}
...
constructor(ox: number, oy: number, ir: number, or: number, sa: number, ea: number, res?: number): this
}
...
declare type Two$BoundingClientRect = {} & Record<"top"
| "left"
| "right"
| "bottom"
| "width"
...
constructor(x: number, y: number, lx: number, ly: number, rx: number, ry: number, command: Two$Commands | string): this;
command: string;
controls: {
right: Two$Vector,
left: Two$Vector
...
constructor(x1: number, y1: number, x2: number, y2: number, stops: $ReadOnlyArray<Two$Stop>): this;
left: Two$Vector;
right: Two$Vector;
spread: string;
...
constructor(x: number, y: number, radius: number, stops: $ReadOnlyArray<Two$Stop>, fx?: number, fy?: number): this;
center: Two$Vector;
radius: number;
focal: Two$Vector;
spread: string;
...
declare type Two$Renderer = Two$SVGRenderer | Two$WebGLRenderer | Two$CanvasRenderer;
