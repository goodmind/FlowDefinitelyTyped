declare module "soupbintcp" {
  ...
  declare export class PacketType {
    ...
    static +LOGIN_ACCEPTED: Class<PacketType__LOGIN_ACCEPTED> &
      PacketType__LOGIN_ACCEPTED &
      65; // 65
    static +LOGIN_REJECTED: Class<PacketType__LOGIN_REJECTED> &
      PacketType__LOGIN_REJECTED &
      74; // 74
    static +SEQUENCED_DATA: Class<PacketType__SEQUENCED_DATA> &
      PacketType__SEQUENCED_DATA &
      83; // 83
    static +SERVER_HEARTBEAT: Class<PacketType__SERVER_HEARTBEAT> &
      PacketType__SERVER_HEARTBEAT &
      72; // 72
    static +END_OF_SESSION: Class<PacketType__END_OF_SESSION> &
      PacketType__END_OF_SESSION &
      90; // 90
    static +LOGIN_REQUEST: Class<PacketType__LOGIN_REQUEST> &
      PacketType__LOGIN_REQUEST &
      76; // 76
    static +UNSEQUENCED_DATA: Class<PacketType__UNSEQUENCED_DATA> &
      PacketType__UNSEQUENCED_DATA &
      85; // 85
    static +CLIENT_HEARTBEAT: Class<PacketType__CLIENT_HEARTBEAT> &
      PacketType__CLIENT_HEARTBEAT &
      82; // 82
    static +LOGOUT_REQUEST: Class<PacketType__LOGOUT_REQUEST> &
      PacketType__LOGOUT_REQUEST &
      79; // 79
  }
  ...
  declare export function formatLoginRequest(
    payload: LoginRequestPayload
  ): Buffer;

  declare export function parseLoginRequest(
    payload: Buffer
  ): LoginRequestPayload;
  ...
  declare export function formatLoginAccepted(
    payload: LoginAcceptedPayload
  ): Buffer;

  declare export function parseLoginAccepted(
    payload: Buffer
  ): LoginAcceptedPayload;

  declare export interface LoginRejectedPayload {
    rejectReasonCode: string;
  }
  declare export function formatLoginRejected(
    payload: LoginRejectedPayload
  ): Buffer;

  declare export function parseLoginRejected(
    payload: Buffer
  ): LoginRejectedPayload;

  declare export class Parser {
    constructor(
      callback: (packetType: PacketType, payload: Buffer) => void
    ): this;
