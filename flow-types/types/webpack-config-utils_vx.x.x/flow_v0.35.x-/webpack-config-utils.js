...
vars?: Array<EnvVars | string>): IfUtils

	declare export function removeEmpty<T>(input: Array<T | void>): T[]

	declare export function removeEmpty<T>(input: $ObjMapi<T, <P>(P) => $ElementType<T, P>>): NonEmptyObject<T>
	...
	declare export type Falsy = false
| ""
| "false"
| void
| null
| 0;
	declare export type DefinedObjKeys<T> = $ElementType<($ObjMapi<T, <P>(P) => "There was Conditional Type, use $Call utility type">), $Keys<T>>;
	declare export type NonEmptyObject<T, P: DefinedObjKeys<T>> = $ObjMapi<{[k: P]: any}, <PP>(PP) => $ElementType<T, PP>>;
	declare export type EnvVars = "production"
| "prod"
| "test"
| "development"
| "dev";
	declare export interface IfUtilsFn {
(value: Y, alternate?: N): Y | N,
(): boolean
} 
