...
onInsertedSemicolon?: (lastTokEnd: number, lastTokEndLoc?: ESTree.acorn$Position) => void,
onTrailingComma?: (lastTokEnd: number, lastTokEndLoc?: ESTree.acorn$Position) => void,
...
endLoc?: ESTree.acorn$Position) => void) | acorn$Comment[],
ranges?: boolean,
...
source: string | null;
constructor(p: acorn$Parser, start: acorn$Position, end: acorn$Position): this
}

declare function acorn$getLineInfo(input: string, offset: number): ESTree.acorn$Position
...
constructor(token: string, isExpr: boolean, preserveSpace: boolean, override: (p: acorn$Parser) => void): this
}
...
declare function acorn$isIdentifierStart(code: number, astral?: boolean): boolean


declare function acorn$isIdentifierChar(code: number, astral?: boolean): boolean
...
declare type acorn$IParse = (input: string, options?: acorn$Options) => ESTree.Program;
...
undefined(): Iterator<acorn$Token>
} 

declare function acorn$tokenizer(input: string, options: acorn$Options): acorn$ITokenizer
...
declare type acorn$ILooseParserClass = (input: string, options?: acorn$Options) => acorn$ILooseParser;
...
acorn$plugins: acorn$PluginsObject): void
