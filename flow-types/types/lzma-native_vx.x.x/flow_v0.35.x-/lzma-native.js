...
timeout?: number
	...
	declare export type Check = "CHECK_CRC32" | "CHECK_CRC64" | "CHECK_NONE" | "CHECK_SHA256";
	declare export type Coders = "easyEncoder"
| "aloneDecoder"
| "rawEncoder"
| "autoDecoder"
| "aloneEncoder"
...
checks: number
	...
	declare export function createStream(coder: Coders, options?: LzmaOptions): JSLzmaStream

	declare export function createCompressor(options?: LzmaOptions): JSLzmaStream

	declare export function createDecompressor(options?: LzmaOptions): JSLzmaStream

	declare export function crc32(input: string, encoding?: string, previous?: number): string
	...
	declare export function Compressor(preset?: Preset, options?: LzmaOptions): JSLzmaStream

	declare export function Decompressor(options?: LzmaOptions): JSLzmaStream

	declare export function parseFileIndex(options: FileOptions, callback?: (err: any, info?: StreamInfo) => void): void

	declare export function parseFileIndexFD(fileDescriptor: number, callback?: (err: any, info?: StreamInfo) => void): void

	declare export function compress(
buf: Buffer | string,
options?: LzmaOptions | Preset,
on_finish?: (result: Buffer) => void): void
...
on_progress?: (progress: number) => void): void,
decompress(
buf: Buffer | string,
on_finish: (result: Buffer) => void,
on_progress?: (progress: number) => void): void
