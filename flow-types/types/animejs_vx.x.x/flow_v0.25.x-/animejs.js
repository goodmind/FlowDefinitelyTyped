declare module "animejs" {
  declare type FunctionBasedParameter = (
    element: HTMLElement,
    index: number,
    length: number
  ) => number;
  ...
  declare type anime$EasingOptions =
    | "linear"
    | "easeInQuad"
    | "easeInCubic"
    | "easeInQuart"
  ...
  declare function anime$remove(
    targets: AnimeTarget | $ReadOnlyArray<AnimeTarget>
  ): void;

  declare function anime$getValue(
    targets: AnimeTarget,
    prop: string
  ): string | number;
  ...
  declare function anime$setDashoffset(
    el: HTMLElement | SVGElement | null
  ): number;

  declare function anime$bezier(
    x1: number,
    y1: number,
    x2: number,
    y2: number
  ...
  declare function anime$timeline(
    params?: anime$AnimeInstanceParams | $ReadOnlyArray<anime$AnimeInstance>
