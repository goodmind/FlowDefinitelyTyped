declare module "graphql-resolve-batch" {
   ...
   */
  declare export function createBatchResolver<
    TSource,
    TReturn,
    TArgs,
    TContext
   ...
   */
  declare export type ResolverFunction<TSource, TArgs, TContext, TReturn> = (
    source: TSource,
    args: TArgs,
    balloontoolbar$context: TContext
  ) => promise$Promise<TReturn>;

  /**
   * A batch function to resolve all fields for the given sources in a single batch.
   */
  declare export type BatchResolveFunction<
    TSource,
    TArgs,
    TContext,
    TReturn
  ...
  ) => TReturn[] | promise$Promise<TReturn[]>;
}
