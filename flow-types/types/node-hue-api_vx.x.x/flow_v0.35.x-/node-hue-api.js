 ...
 */
declare export function upnpSearch(timeout: number): IUpnpSearchResultItem[]
 ...
 */
declare export function nupnpSearch(): Promise<IUpnpSearchResultItem[]>

	declare export class ApiError mixins Error {
name: string;
message: string;
type: any;
address: string;
 ...
 * @param {number} port
 */
constructor(host: string, username: string, timeout?: number, port?: number): this;
 ...
 * @param {lightState.State} stateValues containing the properties and values to set on the light.
 ...
 * @param {Number} id or {String} which is the id of the group to get the details for.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return A promise that will set the specified state on the light, or {null} if a callback was provided.
 */
 ...
 * @param {Array} lightIds An array of light ids to be assigned to the group. If any of the ids are not present in the
...
name: string,
lightIds: string[],
cb: (err: NodeJS.ErrnoException, data: boolean) => void): void;

/**
 * Creates a new light Group.
 * @param name The name of the group that we are creating, limited to 16 characters.
 * @param {Array} lightIds of ids for the lights to be included in the group.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return the id of the group that was created, or null if a callback was provided.
 */
 ...
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return <true> if the deletion was successful, or null if a callback was provided.
 */
 ...
 * @param {String} sceneId The id of the scene to obtain.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 ...
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @returns {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return the result from deleting the scene or null if a callback was provided.
 */
 ...
 * @param {Array} lightIds of ids for the lights to be included in the scene.
 * @param {String} name The name of the scene to be created. If one is not provided, then the id of the scene will become the name.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return the id of the scene that was created (as well as the values that make up the scene),
 ...
 * @param {String} sceneId The id for the scene in the bridge
 * @param scene The configuration of the scene with the details to modify, which can be either a name or an array of
light ids.
 * @param {Boolean} storeLightState flag to save the current light state of the lights in the scene.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return the id of the scene that was updated and the light ids that are now set,
or null if a callback was provided.
 */
updateScene(
sceneId: string | number,
scene: IScene,
storeLightState: boolean): Promise<IScene>;
modifyScene(
sceneId: string | number,
scene: IScene,
storeLightState: boolean): Promise<IScene>;
 ...
 * @param {IState} stateValues containing the properties and values to set on the light.
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
...
stateValues: lightState.State | Object): Promise<IScene>;
 ...
 * @param cb An optional callback function to use if you do not want to use a promise for the results.
 * @return {"NO PRINT IMPLEMENTED: JSDocAllType"} A promise that will return the id of the scene that was created, or null if a callback was provided.
 */
 ...
 * Alias for HueApi
 */
declare export type BridgeApi = HueApi;
	
/**
 * Alias for HueApi
 */
declare export type api = HueApi;
	declare export interface IBridgeVersion {
name: string,
version: {
api: string,
software: string
	...
	declare export interface IUpnpSearchResultItem {
id: string,
ipaddress: string
} 
	declare export interface IBridgeIcon {
mimetype: string,
height: string,
width: string,
depth: string,
url: string
	...
	declare export interface IBridgeDescription {
version: {
major: string,
minor: string
},
...
icons?: IBridgeIcon[]
	...
	declare export type BackupStatus = "idle"
| "startmigration"
| "fileready_disabled"
| "prepare_restore"
| "restoring";
	declare export interface IBridgeConfig {
name: string,
 ...
 */
declare export type ColorMode = "xy" | "ct" | "hs";
 ...
 */
declare export type LightAlert = "none" | "select" | "lselect";
	declare export type LightEffect = "none" | "colorloop";
	
/**
 * If not set when creating a group, default is LightGroup
 */
declare export type LightGroupType = "LightGroup" | "Room" | "Luminaire" | "LightSource";
	
/**
 * If not set when creating a group, default is Other
 */
declare export type RoomType = "Living room"
| "Kitchen"
| "Dining"
| "Bedroom"
| "Kids bedroom"
...
| "Carport"
| "Other";
	declare export interface IScene {
id: string,
name: string,
lights: number[]
} 
	declare export interface ILightGroup {
id: string,
name: string,
type: LightGroupType,
class?: RoomType,
...
lights?: string[]
	...
	declare export interface ILight {
id?: string,
...
"8": string
	...
	declare export interface ILightsApiResponse {
lights: ILight[]
} 
	declare export interface ISensor {
id?: string,
manufacturername: string,
uniqueid?: string,
state: {
...
swversion: string
	...
	declare export interface ISensorsApiResponse {
sensors: ISensor[]
} 
...
[key: string]: ISchedule
	...
	declare export interface IRegisteredUser {
name: string,
username: string,
created: Date,
accessed: Date
} 
	declare export interface INewLightsResponse {
lastscan: string,
[key: string]: string | {
name: string
}
} 
	declare module 'lightState' {
        declare class State  {
payload(): Object;
...
applyRGB(modelid: string): State
	...
	declare function create(values?: Object): State

	declare function isLightState(obj: Object): boolean
