 ...
 */
declare interface WebDriverCookie {
...
args?: any[],
timeout?: number,
pollInterval?: number): (value: any) => Promise<T>
 ...
 * @param {module}
 * :leadfoot/Command|module:leadfoot/Session} parent
 ...
 * @param {"NO PRINT IMPLEMENTED: JSDocFunctionType"} initialiser
 ...
 */
constructor(parent: Command<any> | Session, initialiser?: (setContext: Command.ContextSetter, value: any) => Thenable<T> | T, errback?: (setContext: Command.ContextSetter, error: Error) => Thenable<T> | T): this;
 ...
 * @param {"NO PRINT IMPLEMENTED: JSDocOptionalType"} errback
 * @returns {module}
 * :leadfoot/Command.<any>}
 */
then<U>(
callback: (value: T, setContext?: Command.ContextSetter) => Thenable<U> | U,
errback?: (error: Error, setContext?: Command.ContextSetter) => Thenable<U> | U): Command<U>;
...
callback: (valueOrError: T | Error, setContext?: Command.ContextSetter) => Thenable<U> | U): Command<U>;
 ...
 * @see
 * {@link module:leadfoot/Session#executeAsync} to execute code that returns values asynchronously.
 * @param script
 ...
 * @see
 * {@link module:leadfoot/Session#execute} to execute code that returns values synchronously.
 * @see
 * {@link module:leadfoot/Session#setExecuteAsyncTimeout} to set the time until an asynchronous script is
 ...
 * @see
 * {@link module:leadfoot/Session#setFindTimeout} to set the amount of time it the remote environment
 ...
 * @param button
 * The button to press. See {@link module:leadfoot/Session#click} for available options.
 */
 ...
 * The x-offset in pixels to flick by.
 * @param speed
 * The speed of the flick, in pixels per *second*. Most human flicks are 100–200ms, so
 ...
 * @returns
 * The value of the attribute as a string, or `null` if no such property or
 * attribute exists.
 */
...
depth: number
	...
	declare export interface ContextSetter {
...
constructor(elementId: string | Element | {
ELEMENT: string
 ...
 * @param pathParts
 * Optional placeholder values to inject into the path of the URL.
 */
 ...
 * @returns
 * An object containing arbitrary properties describing the status of the remote
 * server.
 */
getStatus(): Promise<{}>;
...
requiredCapabilities?: leadfoot.Capabilities): Promise<Session>;
 ...
 */
constructor(sessionId: string, server: Server, capabilities: leadfoot.Capabilities): this;
 ...
 * will be negative.
 */
getWindowPosition(windowHandle?: string): Promise<{
x: number,
y: number
}>;
