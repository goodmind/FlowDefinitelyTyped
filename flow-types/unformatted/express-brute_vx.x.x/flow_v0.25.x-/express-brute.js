...
/**
 * @summary
 * Options for {@link MemoryStore} class.
 * @interface
 */
export interface ExpressBrute$MemoryStoreOptions {
...
/**
 * @summary
 * Options for {@link ExpressBrute#getMiddleware} class.
 * @interface
 */
export interface ExpressBrute$Middleware {

/**
 * @summary
 * Allows you to override the value of failCallback for this middleware.
 * @type {Function}
 */
...
/**
 * @summary
 * Disregard IP address when matching requests if set to true. Defaults to false.
 * @type {boolean}
 */
ignoreIP?: boolean,
...
/**
 * @summary
 * Options for {@link ExpressBrute} class.
 * @interface
 */
export interface ExpressBrute$Options {
...
/**
 * @summary
 * Defines whether the lifetime counts from the time of the last request that ExpressBrute didn't prevent for a given IP (true) or from of that IP's first request (false). Useful for allowing limits over fixed periods of time, for example: a limited number of requests per day. (Default: true).
 */
...
/**
 * @summary
 * Gets called with (req, res, next, nextValidRequestDate) when a request is rejected (default: ExpressBrute.FailForbidden)
 */
failCallback?: (
req: express.Request,
 ...
 * @class
 */
declare export class ExpressBrute$MemoryStore  {
 ...
 * The options.
 */
constructor(options?: ExpressBrute$MemoryStoreOptions): this;
 ...
 * The store.
 */
constructor(store: any, options?: ExpressBrute$ExpressBrute$Options): this;

/**
 * @summary
 * Generates middleware that will bounce requests with the same key and IP address that happen faster than the current wait time by calling failCallback.
 * @param {Object} options
 * The options.
 */
getMiddleware(options: ExpressBrute$ExpressBrute$Middleware): express.RequestHandler;

/**
 * @summary
 * Middleware that will bounce requests that happen faster than the current wait time by calling failCallback.
...
next: Function): express.RequestHandler;

/**
 * @summary
 * Resets the wait time between requests back to its initial value.
 * @param {string} ip
 * The IP address.
 * @param {string} key
 * The key. response.
...
reset?: (callback?: () => void) => void
	...
	declare export default typeof ExpressBrute
