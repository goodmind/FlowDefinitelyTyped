...
declare export interface CrossFilter$Bisection<T, U> {
(array: T[], value: U, lo: number, hi: number): number
} 

declare export type CrossFilter$Bisector<T, U> = {
...
initial: () => TValue): CrossFilter$GroupAll<T, TValue>,
...
initial: () => TGroup): CrossFilter$Group<T, TKey, TGroup>,
reduceCount(): CrossFilter$Group<T, TKey, number>,
reduceSum<TGroup>(value: (data: T) => TGroup): CrossFilter$Group<T, TKey, TGroup>,
order(value?: CrossFilter$Selector<TValue>): CrossFilter$Group<T, TKey, TValue>,
...
value: (data: T) => TDimension): CrossFilter$Dimension<T, TDimension>
} 

declare export interface CrossFilter$Dimension<T, TDimension> {
filter(value: TDimension[]): CrossFilter$Dimension<T, TDimension>,
filter(value: TDimension): CrossFilter$Dimension<T, TDimension>,
filter(value: CrossFilter$Selector<TDimension>): CrossFilter$Dimension<T, TDimension>,
...
group<TGroup>(
groupValue: (data: TDimension) => TGroup): CrossFilter$Group<T, TDimension, TGroup>,
